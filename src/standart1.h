...
/// @warning Array не владеющая структура => он не должен аллоцировать память. Даже в случае getPramas() надо возвращать указатель на какой-то внутренний массив даблов, а не аллоцировать, так как будет memory leak
    ...
    struct Color {
        ...
        explicit Color (uint8_t r_, uint8_t g_, uint8_t b_, uint8_t a_):
            r (r_),
            g (g_),
            b (b_),
            a (a_) {}
    ...
    struct Texture {
        ...
        explicit Texture(uint64_t w, uint64_t h, const Color* pix):
            height (h),
            width (w),
    ...
    struct MouseContext {
        ...
        explicit MouseContext(Vec2 pos, int btn): position(pos), button((MouseButton)btn) {}
    };
...
// extern "C" pluginv1::Plugin* getInstance(pluginv1::App *app);